...
## 目录
- [一、总原则：网络分流的唯一权威源](#一总原则网络分流的唯一权威源)
...
1. **已知被墙站点（Google、YouTube、X 等）→ 直接走代理，不做无意义的直连探测**（直连必超时，探测只浪费时间）。先探端口 → 当前 shell 导代理 → `curl` 自测；代理也不可用 → **直接转国内可达渠道**（百度、必应国内版、专业数据库、官方镜像），不反复重试、不强访。"哪些站点算被墙"以各技能实测分流表为准、随实测更新，不凭印象列死清单。
2. **未被墙的境外站点**（含实测可直连的海外 API/CDN，如 Exa、Firecrawl；GitHub/PyPI/Homebrew 等多数时候可直连）→ **先做一次限时约 3 秒的连通性测试**（`curl -I -m 3 <URL>`）：直连通且快就直连（更快更稳），不通或明显慢再挂代理。
3. **国内站点 → 直连**，不要挂代理（部分国内站走代理出口反而失败，如政府站、B站等）。
4. 代理是"可达性工具"，不是"安全开关"——它解决能不能连上，不自动保证内容不泄漏（见第四节）。
...
4. **公共 Wi-Fi**：优先使用蜂窝网络或可信 VPN；坚持 HTTPS；避免在公共网络做敏感登录/支付，确需进行时启用二步验证。
...
│
├─ 目标是已知被墙站点（Google/YouTube/X 等，清单以实测分流表为准）？
│    └─ 直接探端口 → 当前 shell 导代理 → curl 自测；
│       代理不可用 → 转国内可达渠道，不做直连探测、不反复重试、不强访
│
├─ 目标是其他境外站点？
│    ├─ curl 限时约 3 秒测直连
│    ├─ 通且快 → 直连（如 Exa/Firecrawl 等实测可直连的海外 API）
│    └─ 不通/慢 → 探端口导代理后再测；代理也不通 → 转国内可达渠道
│
└─ 国内站点？
     └─ 直连（部分国内站走代理出口反而失败，不必为"用 Google"而强行挂代理）
```
